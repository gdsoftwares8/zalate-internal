    ...
    jwt.verify(req.headers.token, 'fry1egg2secret1json5565b5a5', function (err, decoded) {
        if (decoded) {
            ...
            User.findOne({email: decoded.email}, function (err, firstuser) {
                ...
                if (!err && !firstuser) {
                    res.json({message: 'No error or firstuser'})
                }
                if (firstuser) {
                    if (firstuser.listingsRemaining <= 0) {
                        res.json({message: 'No listing slots remaining'})
                    }
                    if (firstuser.listingsRemaining > 0) {
                        ...
                        newDeal.save(function (err, saveddeal) {
                            ...
                            if (!err && !saveddeal) {
                                res.json({message: 'No error or saveddeal'})
                            }
                            if (saveddeal) {
    ...
    jwt.verify(req.headers.token, 'fry1egg2secret1json5565b5a5', function (err, decoded) {
        if (decoded) {
            ...
            Deal.findOne({id: req.params.id}, function (err, deal) {
                ...
                if (!err && !deal) {
                    res.json({message: 'No deal or err'})
                }
                if (deal) {
                    User.findOne({id: deal.posterId}, function (err, user) {
                        ...
                        if (!err && !user) {
                            res.json({message: 'No error or user found'})
                        }
                        if (user) {
                            if (deal.posterId == decoded.id || decoded.role == 'admin') {
                                Deal.findOneAndRemove({id: req.params.id}, function (err, olddeal) {
                                    ...
                                    if (!err && !olddeal) {
                                        res.json({message: 'No error or old deal'})
                                    }
                                    if (olddeal) {
                                        user.listingsUsed--;
                                        user.listingsRemaining++;

                                        var removeIndex = user.currentListings.map(function (item) {
                                            return item.id;
...
router.get('/browse', function (req, res) {
    ...
    /* jwt.verify(req.headers.token, 'fry1egg2secret1json5565b5a5', function (err, decoded) {
         ...
         if (!err && !decoded) {
             res.json({message: 'no error or decoded'})
         }
         if (err) {
             console.log(err);
         }
